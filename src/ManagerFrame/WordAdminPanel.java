...
public class WordAdminPanel extends JPanel {
	...
	// 위치
	private int startX = 20, startY = 20, startW = 150, startH = 30;
	private int txtStartX = 20, txtStartY = 120, txtStartW = 180, txtStartH = 30;
	private int btnStartX = 55, btnStartY = 310, btnStartW = 100, btnStartH = 20;
	private int labelPY = 65 , inTxtPY = 65, btnPY = 35;
	// 내용물들
	private String [] nameStr = {"     단어정보", "No", "Eng", "Kor",  /**/ "추가", "수정", "삭제", "검색"};
	private JLabel [] label = new JLabel[4];
	private JButton [] btn = new JButton[4];
	private JTextField [] inputTxt = new JTextField[4];
		// 콤보박스
		...
		for(int i=0; i<nameStr.length; i++) {
			if(i<4) {
				...
				if(i==0) {
					...
					inputTxt[i].setBounds(txtStartX, txtStartY, txtStartW, txtStartH);
					selEngKorCombo.setBounds(420, startY, 50, 25);
				} else {
					...
					if(i<3) {
						...
						// 검색을 위한 텍스트필드
						inputTxt[i].setBounds(240, txtStartY += 100, txtStartW -= 25, txtStartH);
					}
				}
